  ...
  <div class="notifications-page">
    <el-card shadow="hover">
      <template #header>
        <div class="page-header">
          ...
          <div class="header-actions">
            ...
            <el-button type="warning" @click="showBroadcastDialog">广播通知</el-button>
          </div>
        </div>
      </template>
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="fromUserId" label="触发用户ID" width="120" />
...
const sendForm = reactive({ userId: '', content: '' })
...
const showSendDialog = () => { sendForm.userId = ''; sendForm.content = ''; sendDialogVisible.value = true }
const showBroadcastDialog = () => { broadcastForm.content = ''; broadcastDialogVisible.value = true }

const handleSend = async () => {
  if (!sendForm.userId || !sendForm.content) { ElMessage.warning('请填写完整'); return }
  sending.value = true
  try {
    await sendNotification({ userId: Number(sendForm.userId), content: sendForm.content })
...
const handleBroadcast = async () => {
  ...
  try {
    await broadcastNotification({ content: broadcastForm.content })
