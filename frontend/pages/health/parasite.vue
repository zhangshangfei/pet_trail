  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        const [petRes, reminderRes] = await Promise.all([
          ...
          uni.$request.get(`/api/pets/${this.petId}/parasite-reminders`)
        ]);
    ...
    async submitForm() {
      ...
      try {
        ...
        let res;
        if (this.isEditing) {
          res = await uni.$request.put(`/api/pets/${this.petId}/parasite-reminders/${this.currentReminder.id}`, submitData);
        } else {
          res = await uni.$request.post(`/api/pets/${this.petId}/parasite-reminders`, submitData);
        }
